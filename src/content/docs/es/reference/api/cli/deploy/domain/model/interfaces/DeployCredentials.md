---
title: "DeployCredentials"
---

[**@aurorajs.dev/catalyst-cli**](/aurora-catalyst-docs/es/reference/api/cli/readme/)

***

[@aurorajs.dev/catalyst-cli](/aurora-catalyst-docs/es/reference/api/cli/readme/) / [deploy/domain/model](/aurora-catalyst-docs/es/reference/api/cli/deploy/domain/model/readme/) / DeployCredentials

# Interface: DeployCredentials

Defined in: [deploy/domain/model.ts:132](https://github.com/avvale/aurora-catalyst-cli/blob/main/src/deploy/domain/model.ts#L132)

Aggregated credentials passed to the adapter factory.

## Properties

### ionos

> **ionos**: [`IonosCredentials`](/aurora-catalyst-docs/es/reference/api/cli/deploy/domain/model/interfaces/ionoscredentials/)

Defined in: [deploy/domain/model.ts:133](https://github.com/avvale/aurora-catalyst-cli/blob/main/src/deploy/domain/model.ts#L133)

***

### plesk

> **plesk**: [`PleskCredentials`](/aurora-catalyst-docs/es/reference/api/cli/deploy/domain/model/interfaces/pleskcredentials/)

Defined in: [deploy/domain/model.ts:134](https://github.com/avvale/aurora-catalyst-cli/blob/main/src/deploy/domain/model.ts#L134)
